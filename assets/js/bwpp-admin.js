/**
 * BWPP admin – live preview, char count, test-post (sends REST nonce).
 */
(() => {
	...
	/* helper – POST to route with form data + nonce */
	const call = (route) => wp.apiRequest({
		path:   route,                                     // ← NO leading “/”
		method: 'POST',
	...
	$btn.on('click', e => {
		e.preventDefault();
		$res.text(''); $spin.addClass('is-active');
